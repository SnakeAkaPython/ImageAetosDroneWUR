...
# To use this script, you need a scihub account obtained a long time in advance.
# The schihub site accepts users registered before the 21st of December 16:46 UTC.
account = 'dirkvoets'
passwd = 'Heimdal1'
...
if len(sys.argv) == 1:
    ...
    print ("        ou : ", prog, " -h")
    print ("example python  %s --lat 50.668639 --lon 5.371120 -a apihub.txt --downloader aria2 "%sys.argv[0])
    print ("example python  %s --lat 50.668639 --lon 5.371120 -a apihub.txt -d 20160801 -f 20160831 -t 31UFS --downloader aria2 "%sys.argv[0])
    sys.exit(-1)
else:
...
if options.start_date!=None:    
    ...
    start_date="20150613" #Sentinel-2 launch date 2015/06/13
...
for prod in products:
    ...
    #==================================download  whole product
        if( cloud<options.max_cloud or (options.sentinel.find("S1")>=0)) and options.tile==None:
            ...
            print (commande_wget)
            if (zipped_file_exists==True):
                print (zipped_file_exists, options.no_download)
            elif (unzipped_file_exists==False and options.no_download==False) or (zipped_file_exists==False and options.no_download==False):
                os.system(commande_wget)
            else:
